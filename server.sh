...
sudo ldapadd -x -w 1111 -D "cn=Manager,dc=devopsldab,dc=com" -f baseldapdomain.ldif
...
sudo ldapadd -x -w 1111 -D "cn=Manager,dc=devopsldab,dc=com" -f group.ldif

slappasswd -s 1111 > temp2

sudo cat > ldapuser.ldif <<EOF 
dn: uid=mkurakevich,ou=People,dc=devopsldab,dc=com
objectClass: top
objectClass: account
...
uidNumber: 1005
gidNumber: 1005
homeDirectory: /home/mkurakevich
...
ldapadd -x -D "cn=Manager,dc=devopsldab,dc=com" -w 1111 -f ldapuser.ldif
